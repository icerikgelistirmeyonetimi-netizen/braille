...
**⚠ OKUMA↔ÖĞRENME MODU GEÇİŞİNDE odak İLK İÇERİĞE (kullanıcı: "okuma modunda başlığa / öğrenme moduna dönünce sayfa altına odaklanıyor, ilk içerik kutusuna odaklanmalı"):** Geçiş rota DEĞİŞTİRMEZ (sadece `okumaModu` state) → `SayfaOdakYonetimi` çalışmaz; geçiş butonu unmount olunca odak gövdeye düşüp ekran okuyucu en üstten (sayfa BAŞLIĞINDAN) okuyordu. Fix: `oncekiOkumaModuRef` + effect (`[okumaModu, bitti]`), İKİ yön:
...
- **Ortak yardımcı `srDurumDuyur(mesaj, ses?)`:** `setSrSiradakiNokta(mesaj)` (bölge auto-okur) + `konusDil(mesaj,{srAtla:true})` (yalnız TTS, `_srBolge`'ye yazMAZ → çift duyuru yok) + opsiyonel `dogruSesi()`/`yanlisSesi()`. `noktayaTikla` 4 durumu bununla duyurur: **yanlış** ("Yanlış. N numara bu hücrede yok." + yanlış sesi), **doğru+kalan nokta** ("Sıradaki nokta: N numara." + **doğru sesi**), **hücre tamam+başka hücre** ("Sonraki hücreye geçiliyor." + doğru sesi), **öğe tamam**. **⚠ "Doğru/doğru" SÖZCÜĞÜ HİÇBİR geri-bildirimde YOK (kullanıcı: "sıradaki nokta yönergesinden sonra bir önceki doğru cevap için doğru diyor"):** mesaj eskiden "**Doğru.** Sıradaki nokta: N" ile başlıyordu → her dokunuşta tekrarlanan "Doğru", hızlı dokununca NVDA kuyruğunda GECİKİP önceki dokunuşunki gibi okunuyordu (DOM temizdi; sorun salt NVDA konuşma kuyruğu + uzun/tekrarlı metin). Fix: olumlu onay = **`dogruSesi()` (ding, kuyruksuz)**; metin kısa + "doğru" sözcüğü yok → gecikme/karışma yok.
