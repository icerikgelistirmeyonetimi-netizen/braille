(async () => {
  const [{
    ...
    import('../src/utils/music-brf/musicVisualLayoutHelpers.js'),
    import('../src/utils/music-brf/musicVisualBarlineHelpers.js'),
  ]);
  ...
  function testDistributeRowExtraWidth() {
    ...
    const total = result.reduce((sum, measure) => sum + measure.layoutWidth, 0) + 20;
  ...
  function testMinimumGapPositions() {
    ...
    const xs = result.positions.map((position) => position.x);
  ...
  function testSkorSatirSagXHesapla() {
    const result = skorSatirSagXHesapla({
      staffRightX: 760,
      rightInset: 0,
    });

    assert(result === 760, `satir right X expected 760, got ${result}`);
  }

  function testFinalBarlineAlignment() {
    const rowRightX = 760;
    ...
    assert(finalX === rowRightX, `final barline expected ${rowRightX}, got ${finalX}`);

    const parts = skorBarlineParcalariAl({
      x: rowRightX,
      type: 'final',
      topY: 64,
      bottomY: 112,
    ...
    const thick = parts.find((part) => part.role === 'final-thick');
    const thin = parts.find((part) => part.role === 'final-thin');

    assert(thick && thick.x === rowRightX, `final thick line expected at ${rowRightX}, got ${thick?.x}`);
    assert(thin && thin.x === rowRightX - 5, `final thin line expected at ${rowRightX - 5}, got ${thin?.x}`);
  }
  ...
  function main() {
    ...
    testSkorSatirSagXHesapla();
    testFinalBarlineAlignment();
